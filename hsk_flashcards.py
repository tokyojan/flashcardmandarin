...
def schedule_sm2(card: Card, q: int) -> None:
    ...
    if q < 3:
        card.reps = 0; card.lapses += 1; card.interval = 1
    else:
        card.reps += 1
        if card.reps == 1: card.interval = 1
        elif card.reps == 2: card.interval = 6
        else: card.interval = int(round(card.interval * card.ease))
...
def format_debug_report(
    ...
    lines.append(f"CEFR selection: {cefr_sel}")
    lines.append("")
...
def load_cards_from_json(
    ...
    for r in data:
        ...
        if lvl in CEFR_ORDER: before_counts[lvl] += 1
        else: before_unknown += 1
    ...
    after_unknown = 0
    for c in final:
        if c.cefr in CEFR_ORDER: after_counts[c.cefr] += 1
        else: after_unknown += 1
...
class App(tk.Tk):
    def __init__(self):
        ...
        try:
            ...
            messagebox.showerror("Deck error", str(e)); self.destroy(); return
    ...
    def _build_widgets(self) -> None:
        ...
        self.cefr_box = ttk.Combobox(top, width=6, textvariable=self.cefr_sel,
                                     values=cefr_values, state="readonly")
        ...
        ttk.Button(top, text="Rebuild Deck (apply filters)",
                   command=self._rebuild_deck, takefocus=0).pack(side="right")
